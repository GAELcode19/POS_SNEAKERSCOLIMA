    ...
    <div class="modal-content" style="max-width:480px;">
        ...
        <form method="POST" action="{{ route('pos.checkout') }}" class="modal-body">
            ...
            <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border-color);">
                <label for="redeem_folio" style="display:block;font-size:13px;color:var(--text-secondary);margin-bottom:6px;">
                    ¿Cliente con saldo a favor de un apartado? Escribe su folio:
                </label>
                <input type="text" name="redeem_folio" id="redeem_folio" class="form-input" placeholder="AP-20260704-0001" style="text-transform:uppercase;font-family:monospace;">
                <small style="color:var(--text-muted);font-size:12px;">Se descontar&aacute; su saldo (v&aacute;lido hasta 1 mes despu&eacute;s de vencer). D&eacute;jalo vac&iacute;o si no aplica.</small>
            </div>
            ...
            <div id="ticketContent">
                ...
                <div style="border-top:2px dashed #ccc;margin-top:12px;padding-top:12px;">
                    <div style="display:flex;justify-content:space-between;font-size:13px;color:#666;margin-bottom:4px;">
                        <span>Subtotal</span>
                        <span>${{ number_format($sale['gross_total'] ?? $sale['total'], 0, '.', ',') }}</span>
                    </div>
                    @if(($sale['credit_applied'] ?? 0) > 0)
                    <div style="display:flex;justify-content:space-between;font-size:13px;color:#059669;margin-bottom:4px;">
                        <span>Saldo a favor ({{ $sale['credit_folio'] }})</span>
                        <span>-${{ number_format($sale['credit_applied'], 0, '.', ',') }}</span>
                    </div>
                    @endif
