...
class PosController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        $request->validate([
            ...
            'redeem_folio' => 'nullable|string',
        ]);

        $grossTotal = array_sum(array_map(fn($item) => $item['price'] * $item['quantity'], $cart));

        // Canje de saldo a favor de un apartado vencido (dentro de 1 mes)
        $creditApplied = 0;
        $redeemedLayaway = null;
        if ($request->filled('redeem_folio')) {
            Layaway::processExpirations();
            $redeemedLayaway = Layaway::where('folio', trim($request->input('redeem_folio')))->first();
            if (!$redeemedLayaway || !$redeemedLayaway->creditIsValid()) {
                return back()->with('error', 'El folio no tiene saldo a favor vigente (no existe, ya fue usado o pasó más de 1 mes desde que venció).');
            }
            $creditApplied = min((float) $redeemedLayaway->paid_amount, $grossTotal);
        }
        ...
        if ($redeemedLayaway && $creditApplied > 0) {
            $newBalance = round((float) $redeemedLayaway->paid_amount - $creditApplied, 2);
            $redeemedLayaway->update([
                'paid_amount' => $newBalance,
                'status' => $newBalance > 0 ? 'vencido' : 'reclamado',
            ]);
        }
        ...
        WhatsAppNotifier::send(
            ...
            "Artículos: {$itemCount}\n" .
            ($creditApplied > 0 ? "Saldo aplicado: -$" . number_format($creditApplied, 0, '.', ',') . "\n" : "") .
            "Total: $" . number_format($total, 0, '.', ',') . "\n" .
            "Pago: " . ucfirst($sale->payment_method) . "\n" .
            now()->format('d/m/Y H:i') . " · Sneakers Colima"
        );
        ...
        return redirect()->route('pos.terminal')->with('sale', [
            ...
            'credit_folio' => $redeemedLayaway?->folio,
            'total' => $total,
    ...
    public function returnProcess(Request $request)
    {
        ...
        WhatsAppNotifier::send(
            "↩️ *Devolución* Orden {$sale->order_number}\n" .
            "Procesó: " . auth()->user()->name . "\n" .
            "Reembolso: $" . number_format($returnTotal, 0, '.', ',') . "\n" .
            "Motivo: " . $reason . "\n" .
            now()->format('d/m/Y H:i') . " · Sneakers Colima"
        );
    ...
    public function label(Request $request, Product $product)
    {
        $copies = max(1, min(100, (int) $request->query('copies', 1)));

        $generator = new BarcodeGeneratorSVG();
        $barcode = $generator->getBarcode($product->sku, $generator::TYPE_CODE_128, 2, 55);

        return view('pos.label', [
            'labels' => [['product' => $product, 'barcode' => $barcode]],
            'copies' => $copies,
            'title' => 'Etiqueta · ' . $product->name,
        ]);
    }

    public function labelsAll(Request $request)
    {
        $brandFilter = $request->query('brand', 'todos');
